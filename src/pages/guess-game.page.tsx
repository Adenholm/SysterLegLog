...
import Card from "../pages2/GuessGame/GuessCard";
import Sound from "react-sound";
import { Categories } from "../components/Categories";
import "../Grid.css";
import Grid from "@mui/material/Grid";
import GuessSettings from "../pages2/GuessGame/GuessSettings";
import "../Settings.css";
...
function GuessPage(props) {
  ...
  useEffect(() => {
    if (allCategories) {
      ...
      !allCategories &&
      categoryStates.every((element) => element === true)
    ) {
  ...
  const fetchCards = async () => {
    ...
    if (
      allCategories == true ||
      getCurrentCategories(categoryStates).length == 0
    ) {
    ...
    try {
      const { data } = await axios.post(
        "http://localhost:4001/cards/guess",
        post_data
      );
      let randomCards = data
  ...
  return (
    <div
      ...
      style={{ marginRight: isSettingsOpen ? "250px" : "0px" }}
    >
      <div>
        ...
        <img
          ...
          style={{ marginRight: isSettingsOpen ? "260px" : "0px" }}
        />
